...
static PyObject *Get_Angles_Asymmetry(PyObject *self, PyObject *args){
	...
	double *phi_LH_one    = calloc(Nfil_,sizeof(double));
    double *phi_LH_two    = calloc(Nfil_,sizeof(double));
	double *phi_LH_final  = calloc(Nfil_,sizeof(double));
	...
	for(int n=0;n<Nfil_;n++){
		...
		if (theta_LH_RMS_ == 0.0){
			//#pragma omp critical
			{
				...
				angles[n*2 + 0] = (double) atan2l((long double) Lhat[n*3+1],(long double) Lhat[n*3+0]);
			}
		}
		else{
			// first, we need to find the phiLH angle with the root solver
            // we check if we have an impossible angle
            if (fabs(random_thetaLH_ptr[n])<fabs(random_psiLH_ptr[n])){
                // unphysical angle
                phi_LH_one[n] = 0.0;
                phi_LH_two[n] = 0.0;
            }
			...
			//#pragma omp critical
			{
				for(j=0;j<3;j++) Lhat[n*3+j] = Lhat0[j]*cos(phi_LH_final[n]) + cross_product[j]*sin(phi_LH_final[n]) + hatZ[j]*dot_product*(1.0 - cos(phi_LH_final[n])) ;
			}
			...
			//#pragma omp critical
			{
				...
				angles[n*2 + 0] = (double) atan2((long double) Lhat[n*3+1],(long double) Lhat[n*3+0]) ;
	...
	PyArray_ENABLEFLAGS((PyArrayObject *)arr_phi_LH_final, NPY_OWNDATA);
    PyArray_ENABLEFLAGS((PyArrayObject *)arr_phi_LH_one, NPY_OWNDATA);
    PyArray_ENABLEFLAGS((PyArrayObject *)arr_phi_LH_two, NPY_OWNDATA);
	...
	PyObject *rtrn = PyTuple_New(9);
	PyTuple_SetItem(rtrn, 0, arr_angles);
	PyTuple_SetItem(rtrn, 1, arr_Lhat);
	PyTuple_SetItem(rtrn, 2, arr_psi_LH);
	PyTuple_SetItem(rtrn, 3, arr_phi_LH_final);
    PyTuple_SetItem(rtrn, 4, arr_phi_LH_one);
    PyTuple_SetItem(rtrn, 5, arr_phi_LH_two);
	PyTuple_SetItem(rtrn, 6, arr_thetaH);
	PyTuple_SetItem(rtrn, 7, arr_thetaL);
...
static PyObject *Paint_Filament_bandpass(PyObject *self, PyObject *args){
    /* Getting the elements */
    PyObject *n = NULL;
    PyObject *nside = NULL;
    ...
    PyObject *betadust_arr = NULL;
    PyObject *Tdust_arr = NULL;
    PyObject *Bandpass = NULL;
    PyObject *N_individual_freqs = NULL;
    ...
    flag = PyArray_IS_C_CONTIGUOUS(Bcube);
    if (!flag){printf("Bcube arr from dict. is not contiguous, exiting ...\n");exit(EXIT_FAILURE);}
    flag = PyArray_IS_C_CONTIGUOUS(Bandpass);
    if (!flag){printf("Bandpass arr from dict. is not contiguous, exiting ...\n");exit(EXIT_FAILURE);}
    ...
    double *Sizes_ptr = PyArray_DATA(Sizes_arr); // Angles is an array with shape (3)
    double *Bcube_ptr = PyArray_DATA(Bcube);
    double *Bandpass_ = PyArray_DATA(Bandpass); // this is an array with shape (Nfreqs,N_individual_freqs,2). In the last axis 0=value of individual frequency, 1=value of bandpass
    int N_individual_freqs_ = (int) PyLong_AsLong(N_individual_freqs);
    ...
    if (isInside==1 && nside_variable >= 128){
        ...
        double *sed_factor_nu = calloc(Nfreqs_,sizeof(double));
        for(int n=0;n<Nfreqs_;n++){
            // second loop around individual freqs
            double weight_sum = 0.0;
			for(int m=0;m<(N_individual_freqs_-1);m++){
                double nu_individual = Bandpass_[n*N_individual_freqs_*2+m*2+0]; //here we want Bandpass_[n,m,0]
                double nu_individual_plusone = Bandpass_[n*N_individual_freqs_*2+(m+1)*2+0]; //here we want Bandpass_[n,m,0]
                double bandpass_individual = Bandpass_[n*N_individual_freqs_*2+m*2+1]; //here we want Bandpass_[n,m,1]
                double x_d_nu = H_PLANCK*nu_individual*1.0e9/(K_BOLTZ*Tdust);
                double x_cmb = H_PLANCK*nu_individual*1.0e9/(K_BOLTZ*T_CMB);
                double thermo_2_rj = pow(x_cmb,2) * exp(x_cmb) / pow(exp(x_cmb) - 1.0,2) ; // multiplying by this factor transform thermo 2 RJ units, divide for the reverse conversion
                sed_factor_nu[n] += (pow(nu_individual*1e9,betadust+1.0)/(exp(x_d_nu)-1.0)/thermo_2_rj/sed_factor_353)*(bandpass_individual)*(nu_individual_plusone-nu_individual)   ;
				weight_sum += (bandpass_individual)*(nu_individual_plusone-nu_individual) ;
            }
			sed_factor_nu[n] /= weight_sum;
        }
        // Now I will run FilamentPaint_CalculateDistances and FilamentPaint_RiemannIntegrator over the tqumap itself
        if (nside_fixed > nside_variable){
            // This means the filament is bigger than the fixed resolution and we have to upgrade the map
...
{"Paint_Filament_bandpass", Paint_Filament_bandpass, METH_VARARGS, NULL},
