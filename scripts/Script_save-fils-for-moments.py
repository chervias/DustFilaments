...
from DustFilaments.FilamentPaint import Paint_Filament_Moments
import time
import yaml
...
if True:
    ...
    for n_rank,n_general in enumerate(fils_indices_split_rank):
        ...
        try:
            ...
            dict_fil_info = []
            tqu_map_fil = Paint_Filament_Moments(n_rank,int(dict['nside']),sizes_rank,centers_rank,angles_rank,float(fpol0_rank[n_rank]),float(thetaH_rank[n_rank]),float(beta_array_rank[n_rank]),float(T_array_rank[n_rank]),Bcube,float(dict['size_box']),Npixels_magfield,int(dict['resolution_low']),int(dict['resolution_high']),freq_array,Nfreqs,tqu_total,skip_Bcube,rank)
            nside_fil = hp.npix2nside(tqu_map_fil.shape[0])
            mask = tqu_map_fil[:,0] > 0.0
            idx_pixels = np.argwhere(mask)
            dict_fil_info = [n_general, nside_fil, float(beta_array_rank[n_rank]), float(T_array_rank[n_rank]), idx_pixels, tqu_map_fil[mask,:] ]
            list_fil_info.append(dict_fil_info)
