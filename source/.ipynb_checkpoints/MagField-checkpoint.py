...
def get_MagField(size,pixels,seed,method,path):
	...
	if method==1:
		...
		Bcube[:,:,:,0] 			= Bpowspec.harm2map(Bharmx,Delta) + Bcube_ls[:,:,:,0]
		Bcube[:,:,:,1] 			= Bpowspec.harm2map(Bharmy,Delta) + Bcube_ls[:,:,:,1]
		Bcube[:,:,:,2] 			= Bpowspec.harm2map(Bharmz,Delta) + Bcube_ls[:,:,:,2]
	...
	if method==3:
		...
		Bharmx,Bharmy,Bharmz 	= Bpowspec.Pk2harm(k,Pk,N,kmax,Deltak,seed)
		Bcube					= np.zeros((pixels,pixels,pixels,3),dtype=np.double)
		Bx = Bpowspec.harm2map(Bharmx,Delta) ; By = Bpowspec.harm2map(Bharmy,Delta) ; Bz = Bpowspec.harm2map(Bharmz,Delta)
