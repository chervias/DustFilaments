...
static PyObject *Paint_Filament(PyObject *self, PyObject *args){
	...
	if (!PyArg_ParseTuple(args, "OOOOOOOOOOOOOOOOOOO",&n, &nside, &Sizes_arr, &Centers_arr, &Angles_arr, &fpol0_arr, &thetaH_arr, &betadust_arr, &Tdust_arr, &Bcube, &size, &Npix_magfield,&resolution,&freqs_arr,&Nfreqs,&tqu_dict,&Nthreads,&skip_Bcube,&rank))
		return NULL;
