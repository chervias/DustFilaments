...
void FilamentPaint_DoLocalTriad(long* ipix_final, int nipix, int nside, double* localtriad, int Nthreads){
...
void FilamentPaint_RiemannIntegrator(double r1, double r2, double* rot_matrix, int idx_pix, double* local_triad, double* centers_arr, double* sizes_arr, double* Bcube_ptr, double size_box, int nbox, double* integ, double fpol0, double thetaH, int n_fil, int skip_Bcube, double* Bcube_center, double sed_factor){
	//Integrator
	double a,b ;
	int i;
	if (r1 > r2){a=r2;b=r1;}
	...
	integ[0] = sed_factor * sumT*deltar ;
	integ[1] = sed_factor * sumQ*deltar ;
	integ[2] = sed_factor * sumU*deltar ;
}