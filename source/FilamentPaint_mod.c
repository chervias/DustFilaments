...
static PyObject *Reject_Big_Filaments(PyObject *self, PyObject *args){
	...
	if (!PyArg_ParseTuple(args, "OOOOOOOOO",&sizes, &thetaL, &size_ratio, &centers, &Nfil, &ell_limit, &Nthreads, &size_scale, &nu_index)) 
		return NULL;
...
static PyObject *Get_Angles(PyObject *self, PyObject *args){
	...
	PyObject *Nthreads = NULL ;
	if (!PyArg_ParseTuple(args, "OOOOOOOOOO",&Nfil, &Bcube, &Npix_box, &random_vector, &random_azimuth, &random_thetaLH, &size, &centers , &theta_LH_RMS, &Nthreads ))
		return NULL;
	int Nfil_ = (int) PyLong_AsLong(Nfil);
...
static PyObject *Paint_Filament(PyObject *self, PyObject *args){
	...
	if (!PyArg_ParseTuple(args, "OOOOOOOOOOOOOOOOOO",&n, &nside, &Sizes_arr, &Centers_arr, &Angles_arr, &fpol0_arr, &thetaH_arr, &betadust_arr, &Tdust_arr, &Bcube, &size, &Npix_magfield,&resolution,&freq,&tqu_dict,&Nthreads,&skip_Bcube,&rank))
		return NULL;
	...
	if (isInside==1 && nside_variable >= 128){
		...
		FilamentPaint_DoLocalTriad(ipix,nipix,nside_variable,localtriad,Nthreads_);
		...
		double freq_ = PyFloat_AsDouble(freq);
		
		double x_d_nu = H_PLANCK*freq_*1.e9/(K_BOLTZ*Tdust);
		double x_d_353 = H_PLANCK*353.0*1.e9/(K_BOLTZ*Tdust);
		double x_cmb = H_PLANCK*freq_*1.0e9/(K_BOLTZ*T_CMB);
		// The conversion is 
		...
		double sed_factor_nu = (pow(freq_*1e9,betadust+1.0)/(exp(x_d_nu)-1.0) / thermo_2_rj ) / sed_factor_353 ;
		...
		if (nside_fixed > nside_variable){
			...
			for(ii=0;ii<nipix;ii++){
				int jj,skip_pix=1; // skip_pixel will be True at the beggining, and FilamentPaint_CalculateDistances will change it to False if we find the intersection
				...
				FilamentPaint_RiemannIntegrator(rDistances[0],rDistances[1],inv_rot_matrix,ii,localtriad,Centers_ptr,Sizes_ptr,Bcube_ptr,Size_double,Npix_magfield_int,integ,fpol0,thetaH,n_fil,skip_Bcube_,Bcenter,sed_factor_nu);
				...
				//{
					...
					for(jj=0;jj<3;jj++) tqu_array_c[jj*((int) npix_variable) + index_pix] += integ[jj];
				//}
			}
			}
		}
			...
			if (nside_fixed == nside_variable){
				...
				for(ii=0;ii<nipix;ii++){
					...
					//{
						...
						for(jj=0;jj<3;jj++) tqu_array_c[jj*npix_fixed + index_pix] += integ[jj];
					//}
				}
				}
			}
				...
				for(ii=0;ii<nipix;ii++){
					...
					#pragma omp critical
					{
						...
						for(jj=0;jj<3;jj++) tqu_array_c[jj*npix_fixed + index_parent_pixel] += integ[jj]/pow(4,step) ;
					}
				}
				}
			}
